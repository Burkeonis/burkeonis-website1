...
import { createDownloadToken, getCommerceBindings, getPaidOrder, getPaidProduct } from "../../lib/commerce";
...
export default async function OrderSuccessPage({ searchParams }: SuccessPageProps) {
  ...
  let productTitle = "your Burkeonis files";
  ...
  if (sessionId && bindings.COMMERCE_DB && bindings.DOWNLOAD_TOKEN_SECRET) {
    ...
    if (order) {
      ...
      productTitle = getPaidProduct(order.productCode)?.title ?? "the Pattern Files";
      state = "ready";
    } else state = "pending";
  ...
  return (
    <main className="commerce-page"><header className="commerce-header"><Link className="wordmark" href="/">BURKEONIS</Link><Link className="text-link" href="/pattern-files">Return to product</Link></header><section className="commerce-shell order-state"><p className="eyebrow">Order status</p>
      {state === "ready" ? <><h1>Payment confirmed.<br />Your files are ready.</h1><p className="commerce-lede">Your access was confirmed by Stripe. This secure download link expires after 15 minutes.</p><a className="button button-primary" href={downloadHref!}>Download {productTitle}</a>{hasOrderBump && <p className="status-note">Your Shadow Work Protocol is included in this download.</p>}</> : state === "pending" ? <><h1>Payment received.<br />Confirming access.</h1><p className="commerce-lede">The order is being verified before files are released. Refresh this page shortly. If access does not appear, contact hello@burkeonis.com with your receipt.</p><Link className="button button-outline" href="/tools.html">Back to the product library</Link></> : <><h1>We need the order link<br />to confirm access.</h1><p className="commerce-lede">Open the confirmation link from your payment return page, or contact hello@burkeonis.com with your receipt.</p><Link className="button button-outline" href="/tools.html">Back to the product library</Link></>}
    </section></main>
  );
}
